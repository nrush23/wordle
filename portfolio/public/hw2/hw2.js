...
function Scene(plink_path = null, music_path = null) {
   ...
   this.update = () => {
      ...
      //If the game state is DROP, calculate collisions and physics
      if (this.DROP) {

         //Our coefficient of restitution is 0.98 (almost perfectly elastic)
         const e = 0.98;
         
         //Obtain our Ball sphere
         const s = this.SPHERES[this.BALL];

         //Calculate the change by gravity and update position from the previous frame
         let a = -9.8 * delta / 2;
         this.uV.y += a;
         let y = s.uO.y + this.uV.y * delta;
         let x = s.uO.x + this.uV.x * delta;

         //Now clamp the ball to not clip into the sphere walls
         x = Math.max(Math.min(x, 1 - 2 * rB), -1 + 2 * rB);

         //Check if the ball has dropped into the gates, if it has reset
         //Otherwise, move ball to its new position
         if (y <= -1 - 2 * rB) {
            this.reset();
         } else {
         ...
         //Now calculate the collisions with each sphere wall and peg
         let S = Array(this.SPHERES.length);
         let P;
         for (let i = 0; i < this.SPHERES.length - 1; i++) {
            P = this.SPHERES[i];
            //Check if the distance between their centers is less than the total sum of their radii
            if (P.checkIntersection(s)) {
               //If it is less than, we have collided.
               //PHYSICS TAKEN FROM: https://vanhunteradams.com/Pico/Galton/Collisions.html#Bouncing-off-a-round-peg,-with-coefficient-of-restitution
               let normal = { x: s.uO.x - P.uO.x, y: s.uO.y - P.uO.y, z: s.uO.z - P.uO.z };
               let mag = magnitude(normal);
               let dist = s.uR + P.uR - mag; //This is how far our ball has clipped into the sphere
               normal = normalize(normal);

               //Push out from peg to prevent clipping
               s.setOrigin(s.uO.x + normal.x * dist, s.uO.y + normal.y * dist, 0.0);
               //Use the constant of restitution, our nomal, and the previous velocity to get our delta v
               let dv = -(1 + e) * dot(this.uV, normal);

               //Multiply our normal by delta V and add it to our previous velocity to get the new velocity after impact
               dv = { x: dv * normal.x, y: dv * normal.y, z: dv * normal.z }
               this.uV.x += dv.x;
               this.uV.y += dv.y;
               this.uV.z += dv.z;

               //Now play our hit sound, normalize its sound by how much it clipped the sphere
               if (this.audio) {
               ...
               //And if a peg was hit, make it glow temporarily
               if (NSW <= i && i < NSW + NSP) {
                  P.uC.a = 0.8;
            ...
            //For the sphere walls, change the glow of their lights over time while the ball is dropping
            if (i < NSW) {
         ...
         //Now pack our ball and set uS in the WebGL
         S[this.BALL] = s.getPack();
         setUniform('4fv', 'uS', S.flat());

         //Turn uTime to the current time to prepare for next frame
         this.uTime = new_time;
      }

      //Now pack the colors for each sphere
      for (let i = 0; i < this.SPHERES.length; i++) {
         const P = this.SPHERES[i];
         if (i < NSW) { //When we are not dropping, the sphere walls just iterate between our different COLORS
            if (!this.DROP) {
               P.uC = this.switchLights(Math.floor(i / 2));
            }
         } else if (NSW <= i < NSW + NSP) { //When we are not dropping, each peg row highlights in a "waterfall" manner
            if (!this.DROP) {
